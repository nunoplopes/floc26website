      ...
      for (const [fileName, assetInfo] of Object.entries(bundle)) {
        ...
        if (/\.(png|jpe?g)$/i.test(fileName)) {
          ...
          if (outputSize >= inputSize) {
            console.log(
              `❌ ${fileName} skip conversion to ${ext}: ${formatSize(inputSize)} → ${formatSize(outputSize)}`,
            );
            continue;
          }
          ...
          const assetId = this.emitFile({
            type: "asset",
            name: fileName.replace(/^assets\//, "").replace(/-[a-z0-9_-]{8}\.[a-z]+$/i, ext),
            source: outputBuffer,
          });
          ...
          console.log(
            `✅ [${ext.slice(1).toUpperCase()}] ${fileName} → ${outputFileName} | ${formatSize(inputSize)} → ${formatSize(outputSize)} (${diff}% smaller)`,
          );
          ...
          try {
            ...
            const diff = ((1 - outputSize / inputSize) * 100).toFixed(1);
            console.log(
              `✅ [SVG]  ${fileName} optimized | ${formatSize(inputSize)} → ${formatSize(outputSize)} (${diff}% smaller)`,
            );
          } catch (e) {
...
export default defineConfig({
  plugins: [tailwindcss(), react(), imageOptimizerPlugin()],
  build: {
    assetsInlineLimit: 0,
  },
});
